...
flyObj.markers <- subset(flyObj.markers, gene %in% conserved$FlyGeneID); nrow(flyObj.markers) # 1139
summary(flyObj.markers$cluster)
# PH 1    PH    PM    PM 120    LM    CC    GST-rich  Adipohemocyte 
# 464     38    32    46        325   69    72          93
#write.table(flyObj.markers, 'gsva_flytohuman_celltype_v1.orthologous.txt', row.names = F, col.names = T, sep = '\t', quote = F)
...
pheatmap(gsvaScores, cluster_rows = F, cluster_cols = F, scale = 'none', treeheight_row = 10, treeheight_col = 10,
         ...
         color = colorRampPalette(c("#3a5fcd", 'white', "#ee0000"))(12), filename = 'gsva_flytohuman_celltype.pdf')
