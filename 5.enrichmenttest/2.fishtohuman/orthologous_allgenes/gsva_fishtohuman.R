...
head(humanexpr, n = 3); dim(humanexpr) # 12310    15
humanexpr$sums <- rowSums(humanexpr)
humanexpr <- subset(humanexpr, sums > 0 )
humanexpr$sums <- NULL
head(humanexpr, n = 3); dim(humanexpr) # 12293    15
...
#write.table(gsvaScores_w, 'gsva_fishtohuman_celltype.hca.txt', quote = F, sep = '\t', row.names = F, col.names = T)
...
pheatmap(gsvaScores, cluster_rows = F, cluster_cols = F, scale = 'none', treeheight_row = 10, treeheight_col = 10,
         ...
         color = colorRampPalette(c("#3a5fcd", 'white', "#ee0000"))(10), filename = 'gsva_fishtohuman_celltype.hca.pdf')
dev.off()
...
humanexpr <- humanexpr[intersect(as.character(conserved$HumanSymbol), rownames(humanexpr)), ]
head(humanexpr, n = 3); dim(humanexpr) # 12035    11
humanexpr$sums <- rowSums(humanexpr)
humanexpr <- subset(humanexpr, sums > 0 )
humanexpr$sums <- NULL
head(humanexpr, n = 3); dim(humanexpr) # 10260    11
...
head(gsvaScores)
gsvaScores_w <- data.frame(celltype = rownames(gsvaScores), gsvaScores, check.rows = F, check.names = F)
#write.table(gsvaScores_w, 'gsva_fishtohuman_celltype.hcl.txt', quote = F, sep = '\t', row.names = F, col.names = T)
min(gsvaScores); max(gsvaScores)
...
pheatmap(gsvaScores, cluster_rows = F, cluster_cols = F, scale = 'none', treeheight_row = 10, treeheight_col = 10,
         ...
         color = colorRampPalette(c("#3a5fcd", 'white', "#ee0000"))(12), filename = 'gsva_fishtohuman_celltype.hcl.pdf')
dev.off()
