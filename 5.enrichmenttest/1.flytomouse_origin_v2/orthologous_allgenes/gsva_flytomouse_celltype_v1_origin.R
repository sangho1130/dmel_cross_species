...
flyObj.markers <- subset(flyObj.markers, gene %in% conserved$FlyGeneID); nrow(flyObj.markers) # 1671
summary(flyObj.markers$cluster)
# PH 1 LG   PH LG   PM LG   PM 120 LG   LM LG   CC LG   GST-rich LG   Adipohemocyte LG 
# 189       25      20      65          137     62      69            73
# PH 1 CI   PH CI   PM CI   PM 120 CI   LM CI   CC CI   GST-rich CI 
# 346       119     45      45          293     98      85 
#write.table(flyObj.markers, 'gsva_flytomouse_celltype_v1_origin.orthologous.txt', row.names = F, col.names = T, sep = '\t', quote = F)
...
head(targetgene_w)
#write.table(targetgene_w, 'gsva_flytomouse_celltype_v1_origin.orthologous.allgenes.MouseIDs.txt', row.names = F, col.names = T, sep = '\t', quote = F)
...
pheatmap(gsvaScores, cluster_rows = F, cluster_cols = F, scale = 'none', treeheight_row = 10, treeheight_col = 10,
         ...
         color = colorRampPalette(c("#3a5fcd", 'white', "#ee0000"))(14), filename = 'gsva_flytomouse_celltype.pdf')
