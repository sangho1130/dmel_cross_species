...
head(conserved); nrow(conserved) # 8710
fishObj.markers <- subset(fishObj.markers, gene %in% conserved$ZebrafishGeneID); nrow(fishObj.markers) # 704
head(fishObj.markers); summary(fishObj.markers$cluster)
# HSCs    Erythroid   Neutrophil  Macrophage    NK/T cell   B cell 
# 34      108         232         274           29          27 
...
### GSVA ###
mouseexpr <- readRDS('../../../../Cross-species_V2/mouse/merged_commongenes_v3/tmp/mouse_pseudobulk_v2.Rds')
mouseexpr <- mouseexpr[intersect(as.character(conserved$MouseSymbol), rownames(mouseexpr)), ]; dim(mouseexpr) # 8713   13
mouseexpr$sums <- rowSums(mouseexpr)
mouseexpr <- subset(mouseexpr, sums > 0 )
mouseexpr$sums <- NULL
head(mouseexpr); dim(mouseexpr) # 8713   13
...
#write.table(gsvaScores_w, 'gsva_fishtomouse_celltype.txt', quote = F, sep = '\t', row.names = F, col.names = T)
...
pheatmap(gsvaScores, cluster_rows = F, cluster_cols = F, scale = 'none', treeheight_row = 10, treeheight_col = 10,
         ...
         color = colorRampPalette(c("#3a5fcd", 'white', "#ee0000"))(12), filename = 'gsva_fishtomouse_celltype.pdf')
