...
flyObj.markers <- subset(flyObj.markers, gene %in% conserved$FlyGeneID); nrow(flyObj.markers) # 1768
summary(flyObj.markers$cluster)
# PH 1 LG   PH LG   PM LG   PM 120 LG   LM LG   CC LG   GST-rich LG   Adipohemocyte LG 
# 208       30      28      62          150     56      67            77 
# PH 1 CI   PH CI   PM CI   PM 120 CI   LM CI   CC CI   GST-rich CI 
# 371       114     51      55          316     102     81 
#write.table(flyObj.markers, 'gsva_flytofish_celltype_v1_origin.orthologous.txt', row.names = F, col.names = T, sep = '\t', quote = F)
...
head(targetgene_w)
#write.table(targetgene_w, 'gsva_flytofish_celltype_v1_origin.orthologous.allgenes.zebrafishIDs.txt', row.names = F, col.names = T, sep = '\t', quote = F)
mySignatures <- GeneSetCollection(object = gs_list) #


### GSVA ###
fishexpr <- readRDS('../../../../Model_species/zebrafish/Tang_Q_et_al/InDrops/tmp/celltype.pseudo.Rds')
...
#write.table(gsvaScores_w, 'gsva_flytofish_celltype_v1_origin.txt', quote = F, sep = '\t', row.names = F, col.names = T)
...
pheatmap(gsvaScores, cluster_rows = F, cluster_cols = F, scale = 'none', treeheight_row = 10, treeheight_col = 10,
         ...
         color = colorRampPalette(c("#3a5fcd", 'white', "#ee0000"))(12), filename = 'gsva_flytofish_celltype_v1_origin.pdf')
