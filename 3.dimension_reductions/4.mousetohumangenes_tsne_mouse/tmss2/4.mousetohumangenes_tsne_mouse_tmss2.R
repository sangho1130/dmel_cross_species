...
mouse_tmss2 <- readRDS('../../../mouse/tabulamuris_ss2/tmp/tm_ss2.harmony.clustered.Rds')
head(mouse_tmss2@meta.data); nrow(mouse_tmss2@meta.data) # 5037
data <- as.matrix(GetAssayData(mouse_tmss2, slot = 'data')); dim(data)
...
for (seed in c(210605821:210605825)){
  set.seed(seed)
  
  ### random sampling ###
  rand_bc <- c()
  for (ct in levels(mouse_tmss2@meta.data$celltype_refined)) {
    tmp <- subset(mouse_tmss2, celltype_refined == ct)
  ...
  label_mini <- mouse_tmss2@meta.data[rownames(data_mini), ]
