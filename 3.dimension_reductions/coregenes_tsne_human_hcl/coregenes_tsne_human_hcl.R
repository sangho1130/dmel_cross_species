...
humanobj <- readRDS('../../human/hcl/tmp/human_hcl.harmony.Rds')
head(humanobj@meta.data); nrow(humanobj@meta.data)
summary(humanobj@meta.data$celltype_refined)
...
for (seed in c(210524161:210524170)){
  set.seed(seed)
  
  ### random sampling ###
  rand_bc <- c()
  for (ct in levels(humanobj@meta.data$celltype_refined)) {
    tmp <- subset(humanobj, celltype_refined == ct)
    use_bc <- sample(rownames(tmp@meta.data), size = round(nrow(tmp@meta.data)/10))
    rand_bc <- append(rand_bc, use_bc)
  }
  #length(rand_bc)
  
  data_mini <- as.matrix(FetchData(humanobj, vars = as.character(conserved$HumanSymbol), cells = rand_bc, slot = 'data'))
  label_mini <- humanobj@meta.data[rownames(data_mini), ]
  head(label_mini)
  ...
  plt <- ggplot(scores, aes(x = tSNE1, y = tSNE2, colour = celltype_refined)) + 
    geom_point() + 
    scale_colour_manual(name="Cell type", values = getPalette(length(levels(humanobj@meta.data$celltype_refined)))) + 
    theme_bw() + 
