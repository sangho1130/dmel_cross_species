...
#dir.create('coregenes_tsne_fly_all')
for (seed in c(210524021:210524025)){
  set.seed(seed)
  ...
  plt <- ggplot(scores, aes(x = tSNE1, y = tSNE2, colour = celltype)) + 
    ...
    labs(title =  ''); plt
  ggsave(paste0(c('coregenes_tsne_fly_all/', seed, '.1-2.pdf'), collapse = ''), units = 'cm', width = 11.5, height = 8)
  ...
  plt <- ggplot(scores, aes(x = tSNE1, y = tSNE3, colour = celltype)) + 
    ...
    theme(axis.text = element_text(colour = 'black'),
          ...
          panel.grid = element_blank()) +
    labs(title =  ''); plt
  ggsave(paste0(c('coregenes_tsne_fly_all/', seed, '.1-3.pdf'), collapse = ''), units = 'cm', width = 11.5, height = 8)
  ...
  plt <- ggplot(scores, aes(x = tSNE2, y = tSNE3, colour = celltype)) + 
    ...
    theme(axis.text = element_text(colour = 'black'),
          ...
          panel.grid = element_blank()) +
    labs(title =  ''); plt
  ggsave(paste0(c('coregenes_tsne_fly_all/', seed, '.2-3.pdf'), collapse = ''), units = 'cm', width = 11.5, height = 8)
  
  write.table(data.frame(Barcode = rownames(scores), scores, check.names = F, check.rows = F), 
              paste0(c('coregenes_tsne_fly_all/', seed, '.txt'), collapse = ''), quote = F, sep = '\t', row.names = F, col.names = T)
}
