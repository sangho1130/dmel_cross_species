...
mouseobj <- readRDS('../../mouse/tabulamuris_10x/tmp/tm_10x.harmony.clustered.Rds')
head(mouseobj@meta.data); nrow(mouseobj@meta.data) # 3652
summary(mouseobj@meta.data$celltype_refined)

conserved <- read.delim('../../2.genelists/stats_orthologous.4847genes.txt')
head(conserved); nrow(conserved) # 4847
...
for (seed in c(210524041:210524045)){
  set.seed(seed)
