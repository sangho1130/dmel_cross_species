...
nrow(fishtohuman_best); length(unique(fishtohuman_best$ZebrafishGeneID)) # 15665 unique genes in fly

freq <- data.frame(table(fishtohuman_best$ZebrafishGeneID))
freq <- subset(freq, Freq > 1); head(freq)

singleton_best <- subset(fishtohuman_best, !ZebrafishGeneID %in% freq$Var1); nrow(singleton_best) # 15138
...
nrow(singleton_best) # 15138
nrow(selected_best) # 365
nrow(evenscores) # 

### many-to-one ###
use_best <- rbind(singleton_best, selected_best)
nrow(use_best); length(unique(use_best$HumanSymbol)) # 15503 fish genes mapped to 12310 human genes

freq <- data.frame(table(use_best$HumanSymbol))
freq <- subset(freq, Freq > 1); head(freq)

onetoone <- subset(use_best, !HumanSymbol %in% freq$Var1); nrow(onetoone) # 9491
...
nrow(onetoone) # 9491
nrow(manytoone_selected) # 2819

orthologous_genes <- rbind(onetoone, manytoone_selected)
nrow(orthologous_genes) # 12310
length(unique(orthologous_genes$ZebrafishGeneID)) # 12310
length(unique(orthologous_genes$HumanSymbol)) # 12310
