...
nrow(flytomouse_best); length(unique(flytomouse_best$FlyGeneID)) # 8758 unique genes in fly

freq <- data.frame(table(flytomouse_best$FlyGeneID))
freq <- subset(freq, Freq > 1); head(freq)

singleton_best <- subset(flytomouse_best, !FlyGeneID %in% freq$Var1); nrow(singleton_best) # 7257
...
nrow(singleton_best) # 7257
nrow(selected_best) # 919

### many-to-one ###
use_best <- rbind(singleton_best, selected_best)
nrow(use_best); length(unique(use_best$MouseSymbol)) # 8176 fly genes mapped to 6470 mouse genes

freq <- data.frame(table(use_best$MouseSymbol))
freq <- subset(freq, Freq > 1); head(freq)

onetoone <- subset(use_best, !MouseSymbol %in% freq$Var1); nrow(onetoone) # 5538
...
nrow(onetoone) # 5538
nrow(manytoone_selected) # 932

orthologous_genes <- rbind(onetoone, manytoone_selected)
nrow(orthologous_genes) # 6470
length(unique(orthologous_genes$FlyGeneID)) # 6470
length(unique(orthologous_genes$MouseSymbol)) # 6470
